...
function App() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      switch(searchType) {
        case 'nom':
          url = `http://localhost:8080/api/employes/recherche/nom?nom=${searchValue}`;
          break;
        case 'departement':
          url = `http://localhost:8080/api/employes/departement/${searchValue}`;
          break;
        case 'salaire':
          url = `http://localhost:8080/api/employes/salaire/min?salaire=${searchValue}`;
          break;
        default:
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`http://localhost:8080/api/employes/${editingId}`, formData);
      } else {
        await axios.post('http://localhost:8080/api/employes', formData);
      }
      fetchAllEmployes();
      resetForm();
    } catch (error) {
